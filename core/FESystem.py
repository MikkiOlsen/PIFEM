...
class FESystem:
    ...
    def solve_batch_factorized(self, F_list: list[np.ndarray], n_workers: int | None = None, extract_dofs=None) -> list:
        ...
        if self.option.Verbose:
            total_elapsed = time.time() - total_start
            avg_per_solve = total_elapsed / n_total
            print(f'Batch factorized ({n_workers} workers): {n_total} solves in {total_elapsed:.2f}s (avg {avg_per_solve:.4f}s per solve)')
        return results
    ...
    def solve_batch_amgx(self, F_list: list[np.ndarray]) -> list[np.ndarray]:
        ...
        avg_per_solve = total_elapsed / n_total
        if self.option.Verbose:
            print(f'Batch AmgX: {n_total} solves in {total_elapsed:.2f}s (avg {avg_per_solve:.4f}s per solve)')
        return results
    ...
    def solve_mumps(self, F: np.ndarray) -> np.ndarray:
        if self._mumps_ctx is None:
            raise RuntimeError("MUMPS context not initialised - call setup_mumps() first.")

        start_time = time.time()
        ctx = self._mumps_ctx
        n_free = len(self._cached_free)

        rhs = np.asfortranarray((F[self._cached_free] - self._cached_correction).astype(np.float64).reshape(n_free, 1))
        ctx._refs['rhs'] = rhs
        ctx.id.nrhs = 1
        ctx.id.lrhs = n_free
    ...
    def solve_batch_mumps(self, F_list: list) -> list:
        ...
        if self.option.Verbose:
            total_elapsed = time.time() - total_start
            avg_per_solve = total_elapsed / n_total
            print(f'Batch MUMPS (NRHS={n_total}): {total_elapsed:.2f}s (avg {avg_per_solve:.4f}s per solve)')
        return solutions
